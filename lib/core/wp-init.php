...
// affiliates record conversion
require_once AFFILIATES_CORE_LIB . '/class-affiliates-conversion.php';
require_once AFFILIATES_CORE_LIB . '/class-affiliates-conversion-commission.php';
require_once AFFILIATES_CORE_LIB . '/class-affiliates-conversion-post-type.php';

// Class for integrate tourmaster
require_once AFFILIATES_CORE_LIB . '/class-affiliates-tourmaster.php';
...
function affiliates_encode_affiliate_id( $affiliate_id ) {
	...
	// Update encoded_id to user_meta
	if ( $encoded_id != get_user_meta( $affiliate_id, 'affiliates_encoded_id', true ) ) {
	    update_user_meta( $affiliate_id, 'affiliates_encoded_id', $encoded_id );
	}	
...
function affiliates_check_affiliate_id_md5( $affiliate_id_md5 ) {
	...
	$query = "SELECT * FROM (SELECT *, md5(ID) as affiliate_id_md5 FROM $table) md5d WHERE affiliate_id_md5 = '$affiliate_id_md5'";
	$affiliate = $wpdb->get_row( $query, OBJECT );
...
function affiliates_admin_menu() {
	...
	// main
	$page = add_menu_page(
		__( 'Tổng quan Affiliates', 'affiliates' ),
		'iHome Affiliates',
	...
	$page = add_submenu_page(
		'affiliates-admin',
		__( 'Chuyển đổi', 'affiliates' ),
		__( 'Chuyển đổi', 'affiliates' ),
		AFFILIATES_ADMINISTER_AFFILIATES,
		'edit.php?post_type=aff_conversion',
		NULL
	);
	$pages[] = $page;
	add_action( 'admin_print_styles-' . $page, 'affiliates_admin_print_styles' );
	...
	$page = add_submenu_page(
		'affiliates-admin',
		__( 'Cài đặt Affiliates', 'affiliates' ),
		__( 'Cài đặt', 'affiliates' ),
...
function affiliates_footer( $render = true ) {
	$footer = '<div class="affiliates-footer">' .
		'<p>' .
		__( 'Bạn đang sử dụng plugin <a style="text-decoration:none;" href="https://zindo.info" target="_blank">iHomeTour Affiliates</a> được phát triển bởi <a style="text-decoration:none;" href="https://zindo.info" target="_blank">Ân</a>.', 'affiliates' ) .
		'</p>' .
		'</div>';
...
/**
 * Override function from tourmaster `table-util.php`
 * @author info@zindo.info
 */
if( !function_exists('tourmaster_insert_booking_data') ){
		function tourmaster_insert_booking_data( $booking_detail, $tour_price, $traveller_amount = 0 ){
			...
			if( empty($booking_detail['tid']) ){
				...
				if( $wpdb->insert("{$wpdb->prefix}tourmaster_order", $data, $format) ){
					// Hook to write a record affiliate
					do_action( 
						'affiliate_record_conversion', 
						array(
...
/**
 * Include script to display ref tree in edit aff screen
 * @author info@zindo.info
 */
function affiliates_admin_print_scripts_treant_tree() {
	...
	wp_enqueue_script( 
		'affiliates-edit-user-page', 
		AFFILIATES_CORE_URL . '/js/affiliates-edit-user-page.js', 
		array('jquery', 
			'treant'
		), 
		NULL, 
...
add_action( 'admin_print_scripts-ihome-affiliates_page_affiliates-admin-affiliates', 'affiliates_admin_print_scripts_treant_tree', 10, 1 );