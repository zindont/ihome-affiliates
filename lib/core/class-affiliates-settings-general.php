	...
	public static function section() {
		...
		if ( isset( $_POST['submit'] ) ) {
			...
			if ( wp_verify_nonce( $_POST[AFFILIATES_ADMIN_SETTINGS_NONCE], 'admin' ) ) {
				...
				$pending_delete_month = !empty( $_POST['pending_delete_month'] ) ? trim( $_POST['pending_delete_month'] ) : get_option( 'pending_delete_month', 2 );
				$old_pending_delete_month = get_option( 'pending_delete_month', 2 );
				if ( $pending_delete_month !== $old_pending_delete_month ) {
					update_option( 'pending_delete_month', $pending_delete_month );
				}				
		...
		$pending_delete_month = get_option( 'pending_delete_month', 2 );
		...
		echo
			'<h3>' . __( 'Thông số đường dẫn Affiliate', 'affiliates' ) . '</h3>' .
			'<p>' .
			'<input class="pname" name="pname" type="text" value="' . esc_attr( $pname ) . '" />' .
			'</p>' .
			'<p>' .
			sprintf( __( 'Thông số hiện tại: <b>%s</b>', 'affiliates' ), $pname ) .
			'</p>' .
			'<p>' .
			sprintf( __( 'Thông số mặc định <em>%s</em>.', 'affiliates' ), AFFILIATES_PNAME ) .
			'</p>' .
			'<p class="description warning">' .
			__( 'Chú ý: Nếu bạn thay đổi thông số này, tất cả các đường link chia sẻ trước đó sẽ không còn tác dụng.', 'affiliates' ) .
			'</p><hr>';

		echo
			'<h3>' . __( 'Dừng tài khoản Affiliate khi không có chuyển đổi (Đơn vị: tháng)', 'affiliates' ) . '</h3>' .
			'<p>' .
			'<input class="pending_delete_month" name="pending_delete_month" type="number" value="' . esc_attr( $pending_delete_month ) . '" />' .
			'</p>';

		// echo
		// 	'<h3>' . __( 'Chuyển hướng', 'affiliates' ) . '</h3>' .
		// 	'<p>' .
		// 	'<label>' .
		// 	sprintf( '<input class="redirect" name="redirect" type="checkbox" %s/>', $redirect ? ' checked="checked" ' : '' ) .
		// 	' ' .
		// 	__( 'Bật chuyển hướng', 'affiliates' ) .
		// 	'</label>' .
		// 	'</p>' .
		// 	'<p class="description">' .
		...
		*/
		echo
			...
			'<input class="button button-primary" type="submit" name="submit" value="' . __( 'Lưu', 'affiliates' ) . '"/>' .
			'</p>' .
			'</div>' .
			'</form>';
