	...
	public static function section() {
		...
		echo
			'<form action="" name="options" method="post">' .
				'<div>' .
				'<h3>' . __( 'Thời gian lưu cookie', 'affiliates' ) . '</h3>' .
				'<p>' .
				'<label>' .
				...
				'</p>' .
				'<p class="description">' .
				__( 'Đây là số ngày kể từ khi khách truy cập truy cập vào trang web của bạn qua link giới thiệu và được tính là hợp lệ.', 'affiliates' ) .
				'</p>' .
				'<p>' .
				__( 'Nếu bạn nhập 0, cookie chỉ có giá trị cho tới khi người dùng tắt trình duyệt (session).', 'affiliates' ) .
				'</p>' .
				'<p>' .
				sprintf(
					__( 'Giá trị mặc định là %d.', 'affiliates' ),
